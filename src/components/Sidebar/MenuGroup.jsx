...
const MenuGroup = ({ item, open, handleClick, index }) => {
    ...
    useEffect(() => {
        setIsActive(() => item?.children?.some((child) => child?.link === pathname));
    ...
    return (
        <div className={`flex flex-col justify-center rounded-xl transition-all ease-linear duration-300`}>
            ...
            <div
                className={`${isActive && 'bg-gray-200 text-indigo-600'} text-gray-400 cursor-pointer group flex items-center justify-between gap-1 rounded-xl text-base p-1 leading-6 font-medium`}
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-linear shadow-black px-2 pl-6 ${open === index ? 'max-h-screen h-fit pb-2' : 'max-h-0 h-0 p-0'}`}>
                <ul className={`border-l ${isActive? 'border-indigo-500':'border-gray-400'}`}>
                    {item.children.map((child, index) => (
                        <li key={index} className="mt-2 flex items-center">
                            <hr className={`w-4 ${isActive? 'border-indigo-500':'border-gray-400'}`} />
